...
func main() {
	...
	Trannspose(outBuffer, gpuBuffer, num_row, 2*num_col)

	// fmt.Println("Executing kernel...")
	// opencl.Trans_pose(outBuffer, gpuBuffer,num_row,num_col )
	// fmt.Println("Waiting for kernel to finish execution...")
	// queue.Finish()
	// fmt.Println("Execution finished.")
	queue.Finish()
	...
	// for i:=0; i<num_col; i++ {
	// 	for j:=0; j < num_row; j++ {
	// 		fmt.Printf("(%f, %f)",results[i][2*j],results[i][2*j+1])
	// 	}
	// }
