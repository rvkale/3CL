...
func FindClfft(X []float32,N int, Direction string) []float32 {
	...
	flag := cl.CLFFTDim1D
	fftPlanHandle, errF := cl.NewCLFFTPlan(context, flag, []int{N}) //Don't change it 2*N
	if errF != nil {
...
func main() {
	...
	PartAFFT := FindClfft(PartA, N, "frw")
	...
	PartBFFT := FindClfft(PartAFFT, N, "inv")
